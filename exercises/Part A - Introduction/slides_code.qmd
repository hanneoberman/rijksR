...
format: 
  revealjs:
    ...
    progress: true
    logo: ../img/UU_logo.png
    footer: "CC BY-NC-SA 4.0 --- hanneoberman.github.io/rijksR"
    slide-number: true
...
<!-- - convert to quarto -->
<!-- - add to website -->
<!-- - replace img of console etc. -->

## Goal of this course
- Gentle introduction to:
  - Programming languages
  - R for data science
- Provide a foundation for the [advanced R course](https://www.gerkovink.com/rijkR/)
...
##### Why R:
- Designed for statistics/data science (=easy pipelines)
- Huge community of users (=support)
- Integration between libraries (=everything just works)
...
- Your computer finds files by their path:
  - `/Users/yourname/Desktop/someproject/data/somefile.csv` (Mac/Linux)
  - `C:\\Users\\yourname\\Desktop\\someproject\\data\\somefile.csv` (Windows)
...
- Absolute (e.g., the ones just shown)
    - The "full" path
    - Defined from the root directory, the top-most directory on your machine

- Relative (e.g. `data/somefile.csv`)
    - Only the "relevant" piece of the path
    - Defined from the working directory
    - Makes it possible to use the code in different computers
    - Interpreted as: in the current working directory, find the folder "data", and inside, the file "somefile.csv"
...
- Working directory:
  - If not associated to a project: your home directory (e.g. `/Users/yourname`) 
...
- Creating variables in R is very straightforward: 
    - you just use the assignment operator, `<-`
...
- Alternatively, the internet will tell you almost everything you'd like to know and sites such as http://www.stackoverflow.com and http://www.stackexchange.com, as well as `Google` and `LLM` can be of tremendous help. 
    ...
    - On social media, the #RStats is often used to discuss R
...
```{r, eval = FALSE}
# Install a package to read `csv` files
install.packages("readr") 
```

```{r, eval = TRUE, echo = TRUE, collapse = TRUE}
# Loading the package, usually at the top of the R script
library(readr)
```
